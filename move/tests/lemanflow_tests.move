...
module sui_hackathon::lemanflow_tests {
    ...
    fun test_register_passport() {
        ...
        {
            let clock = clock::for_testing(&mut scenario);
    ...
    fun test_double_claim_failure() {
        ...
        {
            let clock = clock::for_testing(&mut scenario);
            passport::register_passport(USER1, &clock, ts::ctx(&mut scenario));
        };
    ...
    fun test_multiple_users() {
        ...
        {
            let clock = clock::for_testing(&mut scenario);
            passport::register_passport(USER2, &clock, ts::ctx(&mut scenario));
        };
