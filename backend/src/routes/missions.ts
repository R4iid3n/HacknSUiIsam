...
export async function missionRoutes(
  ...
  fastify.get<{ Querystring: { eventId: string } }>('/api/missions', async (request, reply) => {
    try {
      ...
      const eventInfo = {
        eventId,
        name: eventFields?.name || 'Unknown Event',
        description: eventFields?.description || '',
        startTime: parseInt(eventFields?.start_time || '0', 10),
        endTime: parseInt(eventFields?.end_time || '0', 10),
        active: eventFields?.active || false,
      };
      ...
      return { event: eventInfo, missions };
    } catch (error: any) {
