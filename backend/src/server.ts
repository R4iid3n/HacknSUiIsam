...
async function start() {
  ...
  let aiAgentService: AIAgentService | MockAIAgentService;
  if (config.mockMode) {
    aiAgentService = new MockAIAgentService();
  } else {
    aiAgentService = new AIAgentService(suiClient, config.packageId, sponsorTxService.getSponsorKeypair());
  }
  ...
  console.log(`   - AI Agents: ENABLED (automatisation des workflows)`);
  console.log('');
  ...
  await aiAgentRoutes(fastify, config, aiAgentService);
